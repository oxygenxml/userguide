...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" 
  ...
  <sch:pattern>
    <sch:rule context="*[contains(@class, ' topic/li ')]">
      <sch:let name="paragraphs" value="count(*[substring-before(substring-after(@class, ' '), ' ')='topic/p'])"/>
      <sch:report test="$paragraphs>0 and count(text()[normalize-space(.)!=''])>0" role="error" sqf:fix="wrapTextInP"> 
        You should wrap the text that you entered directly inside the <sch:name/> element in a "p"
        element. </sch:report>
      
      <sqf:fix id="wrapTextInP">
        <sqf:description>
      ...
      <sch:report test="$paragraphs>0 and element()[substring-before(substring-after(@class, ' '), ' ')!='topic/p']" 
        role="error"> 
        You should wrap the element that you entered directly inside the <sch:name/> element in a "p"
        element. </sch:report>
